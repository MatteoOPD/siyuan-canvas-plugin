...
class CanvasView {
  ...
  async init(){ this.element.innerHTML=`<div class="syc-toolbar"><button data-action="ref">+ Referenz</button><button data-action="text">+ Text</button><button data-action="link">Verbinden</button><button data-action="delete">Löschen</button><span></span><button data-action="save">Canvas speichern</button></div><div class="syc-main"><div class="syc-viewport"><svg class="syc-edges"><defs><marker id="syc-arrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L9,3 z"/></marker></defs></svg><div class="syc-world"></div></div><aside class="syc-inspector"><h3>Canvas</h3><p>Karte auswählen, um sie zu bearbeiten.</p></aside></div>`; try{this.graph=await this.store.load(this.canvasId)||empty(this.canvasId);}catch(error){showMessage(`Canvas-Speicher nicht erreichbar: ${String(error)}`);} this.bind(); await this.render(); }
...
export default class SiYuanCanvas extends Plugin { private store=new Store(); private tabType="siyuan-canvas";
  onload(){this.addIcons(`<symbol id="iconSiYuanCanvas" viewBox="0 0 32 32"><path d="M5 7h9v9H5zM18 16h9v9h-9zM14 11l4 5M14 16l4 4" fill="none" stroke="currentColor" stroke-width="2"/></symbol>`);const store=this.store;this.addTab({type:this.tabType,init(this:any){const canvasId=this.data?.canvasId||`canvas-${Date.now()}`;new CanvasView(this.element,canvasId,store).init().catch((error:unknown)=>{console.error("SiYuan Canvas konnte nicht initialisiert werden",error);this.element.innerHTML=`<div class="syc-error">Canvas konnte nicht geladen werden: ${String(error)}</div>`;});}});}
  ...
  private open(canvasId:string){openTab({app:this.app,custom:{icon:"iconSiYuanCanvas",title:"Canvas",data:{canvasId},id:`${this.name}${this.tabType}`}});}
}
